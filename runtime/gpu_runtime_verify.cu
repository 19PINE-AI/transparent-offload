...
struct Conn { cudaStream_t st; cudaEvent_t ev; uint8_t *h,*d; int waiting; int finished; long ops; fw_fiber fb; unsigned char* expect; };
...
static void body(void*arg){
    ...
    while(!stop){
        ...
        if(memcmp(c.h,c.expect,MSG)!=0) __sync_fetch_and_add(&g_mismatch,1);
        c.ops++;
    }
    c.finished=1;
}
...
int main(int argc,char**argv){
    ...
    for(int i=0;i<N;i++){ conns[i].expect=(unsigned char*)malloc(MSG); unsigned char tmp[8192];
      memset(tmp,(unsigned char)(i+1),MSG); EVP_CIPHER_CTX*x=EVP_CIPHER_CTX_new(); unsigned char iv2[16]={0};
      EVP_EncryptInit_ex(x,EVP_aes_128_ctr(),0,key,iv2); int ol2; EVP_EncryptUpdate(x,conns[i].expect,&ol2,tmp,MSG); EVP_CIPHER_CTX_free(x);} 
    ...
    pthread_join(tmr,0);
    printf("%s,N=%d,MSG=%d,ops=%ld,tput=%.0f,MISMATCHES=%ld\n", mode,N,MSG,ops,ops/dt,g_mismatch);
    return 0;
}
