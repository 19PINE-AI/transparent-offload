...
several microseconds, and a fine-grained offload also takes from a few to tens of microseconds. Such
an offload might be AES on a few kilobytes, a small inference, or an HSM round-trip. This is the
...
no emulated latencies, the edit recovers $1.2$--$5\times$ with no failed requests. It is bounded at
...
Overlap introduces one correctness hazard, and we address it. The post-processing after an
offload may mutate shared state, such as a global counter, a cache, or a key in a key-value store. In
that case, overlapping two requests can reorder their read-modify-write sequences and corrupt that
state. We provide a transparent conflict detector, based on page protection and version snapshots,
that flags exactly these conflicts and serializes only the conflicting work. It applies at both ends
of the spectrum and needs no application annotation.
...
kilobytes completes in tens of microseconds. A compression card or a small on-GPU inference takes
tens to hundreds of microseconds. A hardware security module signature, a post-quantum
key-encapsulation, or a remote inference call takes milliseconds. What unites them
(Figure~\ref{fig:landscape}) is that they are \emph{fine-grained} relative to operating-system
scheduling: the offload finishes on a timescale comparable to, or only modestly larger than, a
context switch. In this regime the classic ways of waiting are wasteful, as Figure~\ref{fig:pipeline}
illustrates. Blocking pays a switch and a wakeup that rival the offload, and busy-waiting pays a whole
core. Overlapping the offload with other work is the only approach that neither wastes the CPU nor
inflates latency, and it is the approach this paper makes easy to adopt.
...
serving are thus the ones with the most to gain from overlap. As we show next, they are also the ones
a transparent runtime cannot help.
...
(Figure~\ref{fig:condsweep}), a reusable practitioner lesson. We resolved several other obstacles,
each once. The server's I/O may use \texttt{recv}/\texttt{send}/\texttt{poll} rather than
\texttt{read}/\texttt{write}. \texttt{pthread\_self} must return the real OS thread identity so that
the C library's stack-bounds logic works. A daemon's \texttt{fork} drops the carrier thread, which
must be re-established. And pinning the carrier to a real-time priority can invert priorities against
a lock holder.
...
touches existing code is memcached~\cite{memcached}, which has no asynchronous-command interface. We
add a connection state and the park/resume transition into its state machine, for 70 lines added and
one modified. memcached is thus the exception that proves the rule: the line count is dominated by
whether the server already exposes a deferred-response primitive, a point we formalize next.
...
fully blocked loop back into full concurrency, and the win grows with offload weight up to the
accelerator's throughput ceiling. An \emph{event loop with a worker pool} stalls only one loop of
several, so the win is \emph{large} but bounded by the number of loops. A \emph{thread or goroutine
pool} overlaps offloads \emph{automatically}, with zero asynchronous code, up to the pool size. The
runtime is already doing the work. A \emph{proxy} with a native offload engine treats async offload as a
...
post-processing after an offload performs a read-modify-write on a global such as a request counter,
a cache entry, or a key in a store, overlapping two requests can interleave their read-modify-write
...
during the park as a conflict. Under enforcement it holds a lightweight handler lock from the read to
the write across the offload, so that only \emph{conflicting} handlers serialize while everyone else
stays overlapped. On the stock-Redis workload the detector catches essentially every conflict ($26{,}185$
...
\paragraph{Setup.}
All offloads are real, run on real hardware, with no emulated latencies. Experiments run on a server
with an NVIDIA RTX PRO 6000 (Blackwell) GPU. The GPU-offload path performs AES~\cite{openssl} on its
own CUDA stream with completion polled by \texttt{cudaEventQuery}~\cite{cudastreams}, with the block
size sweeping from $4$\,KiB (light, launch-bound) to $8$\,MiB (heavy, bandwidth-bound at $\sim$$2.3$\,ms).
...
bottleneck: a GIL-bound Python agent gains nothing ($0.9\times$, as its per-request protocol handling
...
(at $2.1\times$ the proxy's GPU still has headroom, because the agent's protocol cost, not the device,
caps it).
...
tail (p99) latency low to roughly \emph{four} times the offered load that the blocking path can. The
median knee is near $410$\,K req/s versus $103$\,K for blocking (Figure~\ref{fig:latency}), the
...
wastes, keeping the GPU busy while the synchronous path idles it between requests. It cannot, however,
exceed the device's throughput (a compute-bound kernel would saturate even sooner). A
\emph{latency-bound} offload lifts that device ceiling, because its device is not saturated and many
requests can be in flight at once. We build a real remote signer over TCP (a genuine RSA-2048
signature per request, $821$\,\textmu s round-trip), and the same Python server reaches $3.53\times$
($2038\to7202$\,req/s). Notably this is \emph{not} the order-of-magnitude win a deep device queue
could in principle give. The bottleneck has simply moved from the device to the \emph{server's own
overlap capacity}. The \texttt{asyncio} executor keeps only $\sim$$7$ requests genuinely in flight
under the GIL. Real overlap on a stock server is thus bounded at both ends, by the accelerator's
throughput \emph{and} by how much concurrency the server can actually express. Across our servers and
offloads it lands at $1.2$--$5\times$ (and $17.3\times$ only for the purpose-built transparent fiber
runtime).
...
\emph{across} connections for free, since the OS runs the backends concurrently and the GPU is already
fed. The only edit-addressable win is \emph{within} a single query that pipelines many offloads.
With a real GPU AES offload this intra-query pipelining is modest: PostgreSQL $1.28\times$ and
MariaDB $1.9\times$ through a $\sim$$30$-line user-defined function with no core edits. This is
precisely because the launch-bound GPU op is light: once the device is fed, pipelining more of them
inside one query cannot exceed its throughput. It is the same edit, the same GPU, and the same per-row
work, and the win is set by the device, exactly as the model predicts.
...
efficiently. Examples include dataplane operating systems (IX~\cite{ix}), work-stealing for tail
latency (ZygOS~\cite{zygos}), and core-reallocating runtimes (Shenango~\cite{shenango},
Caladan~\cite{caladan}, Demikernel~\cite{demikernel}), typically over kernel-bypass I/O
...
\texttt{async}/\texttt{await}) make overlap the default, but only if one writes the application in
them from the start. Our concern is the opposite: injecting overlap into \emph{existing} servers with
minimal change. Proxy offload engines (HAProxy's SPOE~\cite{spoe}, Envoy's \texttt{ext\_proc}~\cite{extproc})
make async offload a first-class, configuration-only feature for the proxy tier. We measure the
HAProxy case directly.
...
GPU offload saturates the device's bandwidth) and by the server's own overlap capacity (even a real
latency-bound remote signer tops out at $3.5\times$ behind a GIL-bound executor). So the
order-of-magnitude wins an unbounded offload model predicts do not survive real hardware, except in
the purpose-built transparent runtime ($17.3\times$). A simple model, the concurrency model crossed
with offload weight, predicts both the speedup and the code cost in advance. A transparent conflict
detector, validated on a stock Redis server, keeps the overlap correct when it touches shared state. The result is a practical recipe, and a map, for hiding
